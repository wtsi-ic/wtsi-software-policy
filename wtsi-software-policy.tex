...
status of the work (see section \ref{section:impnotes.records} for best practices in 
this regard). 
...
under the terms of this clause (e.g. you can write a declaration that says 
...
\par If an existing source file does not contain any GRL copyright 
notice, you should add a new copyright line following any existing copyright notices 
and including the year in which the modifications were created. If the existing source 
...
dedicated to the public domain by the author. In that instance, ensure 
...
work which was placed into the public domain (in that case, give the name of the 
author and include a copy of their written public domain dedication). 

\par Note that it is important to comply with the terms of license under 
which source files are received from external sources (see 
\ref{section:policy.protection}). In some cases that may limit under which license(s) 
it is acceptable to distribute a combined work which includes externally developed 
source code and GRL copyrighted code. In extreme cases, it may even be 
impossible to combine multiple pieces of third-party code under different 
licenses without violating the licensing terms of at least one of them (for example 
GPLv2 code cannot be combined with Apache 2.0 code), so 
please be aware of any license restrictions before you start using any third-party 
source code or libraries. Note that it is possible to combine material available 
under multiple different third-party licenses provided they are all compatible with 
the license under which the combined work is distributed. 
...
or svn) that allows individual line-level contributions to be attributed to specific 
individuals. If contributions are accepted from third-parties, they should also 
indicate their authorship within each file (in addition to any copyright notices 
they may add as appropriate).
...
one of the acceptable free software licenses applied to your code so that it is 
clear to recipients what the license terms are under which they are receiving 
the software. 
...
the software causes any unintention harm to the licensee. 
...
user interaction if you want to ensure that any future use of the software that does provide 
remote user interaction will be required to provide any modifications that have been made 
to your source code to the users of that service. For example, software for genomic analysis 
...
to simply add the current year to a file unless you have made a new creative 
contribution to it. Use any available information (e.g. source control records, 
...
involved (i.e. if you know the original date the file was created but are not sure 
